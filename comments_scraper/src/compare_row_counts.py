...
# Also insurance against repeated days of 0 new rows -> problem with Cognos or SimpleSurvey
total_db = sum([tup[1] for tup in course_code_count_db])
total_pd = comments.shape[0]
new_rows = total_pd - total_db
logger.info('{0} new comments today.'.format(new_rows))
