...
from registrations_scraper.config.directories import PICKLE_DIR
from registrations_scraper.src.geocode_registrations import get_lat_lng
...
df['offering_lat'] = df.apply(lambda x: get_lat_lng(x['offering_city_en'], x['offering_province_en'], geo_dict, new_ctr, fail_ctr)['lat'], axis=1)
df['offering_lng'] = df.apply(lambda x: get_lat_lng(x['offering_city_en'], x['offering_province_en'], geo_dict, new_ctr, fail_ctr)['lng'], axis=1)
